...
async function loadMaps(key) {
  ...
  await new Promise((resolve, reject) => {
    ...
    script.onerror = () => {
      reject(
        new Error("โหลด Google Maps ไม่สำเร็จ กรุณาตรวจ API Key")
      );
    };
...
async function start() {
  ...
  try {
    ...
    /*
     * สำคัญ:
     * ซ่อนหน้าใส่ API Key ทันทีที่แผนที่พร้อม
     * ไม่ต้องรอการค้นหา LPG รอบแรก
     */
...
async function init() {
  ...
  if (!Place || !AdvancedMarkerElement) {
    throw new Error(
      "เบราว์เซอร์โหลด Places หรือ Marker library ไม่สำเร็จ"
    );
  }
  ...
  /*
   * เริ่มค้นหา LPG ต่อเบื้องหลัง
   * ไม่ block การซ่อนหน้า API Key
   */
  searchArea(true).catch((error) => {
    console.error(
      "ค้นหา LPG รอบแรกไม่สำเร็จ:",
      error
    );
  });
}
...
function buildSearchCenters(bounds) {
  ...
  /*
   * Nearby Search มีรัศมีสูงสุด 50 km
   * จึงแบ่ง viewport เป็น cell
   */
...
async function searchArea(force = false) {
  ...
  if (zoom < 8) {
    ...
    $("count").textContent =
      "ซูมเข้าอีกนิดเพื่อค้นหา LPG";

    status(
      "ซูมเข้าเพื่อให้ค้นหาสถานี LPG ได้ละเอียดขึ้น",
      3000
    );
  ...
  try {
    ...
    if (
      ...
      status(
        "Google Places ไม่ตอบข้อมูล กรุณาตรวจ API Key หรือโควตา",
        5000
      );
    } else {
      $("count").textContent =
        `LPG ในพื้นที่นี้ ${visible.length} แห่ง`;

      if (failures) {
        status(
          `ค้นหาได้บางส่วน (${failures} จุดค้นหามีปัญหา)`,
          3500
        );
      }
    }
...
function dedupePlaces(places) {
  ...
  for (const p of places) {
    const key =
      p.id ||
      `${p.location?.lat?.()}:${p.location?.lng?.()}:${p.displayName?.text || p.displayName || ""}`;
...
function hasLpgFuelData(place) {
  return getFuelPrices(place).some(
    (fuel) =>
      String(fuel.type || "")
        .toUpperCase() === "LPG"
  );
...
function hasStrongLpgName(place) {
  ...
  return (
    /\blpg\b|แก๊ส lpg|ก๊าซ lpg|ปั๊ม lpg|lpg station/
      .test(text)
  );
}
...
function isActualLpg(place) {
  return (
    hasLpgFuelData(place) ||
    hasStrongLpgName(place)
  );
}
...
function openStation(place) {
  ...
  let priceHtml =
    `<div class="meta"><b>ราคา LPG:</b> ไม่มีข้อมูลราคา</div>`;
  ...
  if (lpg?.price) {
    ...
    priceHtml =
      `<div class="meta"><b>ราคา LPG:</b> ${esc(price || "มีข้อมูลราคา")}</div>` +
      (
        updated
          ? `<div class="updated">อัปเดต/ตรวจพบ: ${esc(updated)}</div>`
          : ""
      );
  }
...
function locate() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (error) => {
      const msg =
        error.code === 1
          ? "กรุณาอนุญาต Location ให้เว็บนี้"
          : "หาตำแหน่ง GPS ไม่สำเร็จ";
...
async function searchText(query) {
  ...
  try {
    ...
    for (const place of places || []) {
      ...
      button.innerHTML =
        `<b>${esc(place.displayName?.text || place.displayName || "")}</b>` +
        `<small>${esc(place.formattedAddress || "")}</small>`;
